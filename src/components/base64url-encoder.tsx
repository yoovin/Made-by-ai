...
import { Base64UrlToolResult, decodeBase64UrlInput, encodeBase64UrlInput } from "@/lib/base64url-tools";

const ENCODE_EXAMPLE = '{"alg":"HS256","typ":"JWT"}';
const DECODE_EXAMPLE = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9";
const INVALID_EXAMPLE = "SGVsbG8/";
...
export function Base64UrlEncoder() {
  ...
  const [result, setResult] = useState<Base64UrlToolResult | null>(null);
  const [copyStatus, setCopyStatus] = useState<CopyStatus>("idle");

  function handleEncode() {
    setCopyStatus("idle");
    setResult(encodeBase64UrlInput(input));
  }

  function handleDecode() {
    setCopyStatus("idle");
    setResult(decodeBase64UrlInput(input));
  }
  ...
  return (
    <section className="panel">
      <h2 className="section-title">Base64URL 입력</h2>
      <textarea
        ...
        <div className={`json-formatter-result ${result.ok ? "ok" : "error"}`}>
          {result.ok ? (
            <>
              ...
              {copyStatus === "copied" ? (
                <p className="section-desc">{result.action === "encode" ? "인코딩 결과를 클립보드에 복사했습니다." : "디코딩 결과를 클립보드에 복사했습니다."}</p>
              ) : null}
